...
func (ec *executionContext) _Mutation_createPreUser(ctx context.Context, field graphql.CollectedField) (ret graphql.Marshaler) {
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (interface{}, error) {
		...
		if data, ok := tmp.(*model.PreUser); ok {
			return data, nil
		}
		return nil, fmt.Errorf(`unexpected type %T from directive, should be *github.com/diogoX451/inventory-management-api/internal/graph/model.PreUser`, tmp)
	})
	if err != nil {
		ec.Error(ctx, err)
		return graphql.Null
